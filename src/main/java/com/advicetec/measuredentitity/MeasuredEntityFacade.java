...
public final class MeasuredEntityFacade {
	...
	private HashMap<String,Object> attLasValueMap;
								 ...
								 String unit1PerCycles, String unit2PerCycles,  String actualProductionCountId, Integer purgeFacadeCacheMapEntries)
	{
		...
		this.attLasValueMap = new HashMap<String, Object>();
	...
	public synchronized void setAttributeValue(AttributeValue attrValue)
	{
		...
		String attrValueKey = attrValue.getGenerator()+"~"+attrValue.getGeneratorType()+"~"+mav.getAttr().getName();
		Object lastAttrValue = this.attLasValueMap.get(attrValueKey);
		if(lastAttrValue != null &&  lastAttrValue.toString().compareTo(mav.getValue().toString()) == 0){
			logger.warn("The attribute:" + mav.getAttr().getName() +" has same value["+mav.getValue().toString()+"]["+lastAttrValue.toString()+"] !!!");
		}else{
			this.attLasValueMap.put(attrValueKey,mav.getValue());
